...
public class GuiHandler implements IGuiHandler {
    ...
    public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID >= 0) { //normal gui
            ...
            int[] upgrades = IronBackpacksHelper.getUpgradesAppliedFromNBT(backpack);
            return GUIBackpackAlternate.GUI.buildGUIAlternate(player, new InventoryAlternateGui(player, backpack, BackpackTypes.values()[Math.abs((ID + 1))]), upgrades, BackpackTypes.values()[Math.abs((ID + 1))]);
        }
        return null;
    }
}
