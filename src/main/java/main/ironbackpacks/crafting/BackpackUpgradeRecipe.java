...
public class BackpackUpgradeRecipe extends ShapelessOreRecipe {
    ...
    private boolean canApplyUpgrade(ItemUpgradeBase upgradeToApplyBase, int[] upgrades, int totalUpgradePoints){
        ...
        }else if(upgradeToApplyBase.getTypeID() == IronBackpacksConstants.Upgrades.DEPTH_UPGRADE_ID){ //If trying to apply the depth upgrade you need a nesting upgrade applied already
            for (int upgrade : upgrades){
